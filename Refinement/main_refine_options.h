...
static struct option main_refine_long_options[] =  {
...
class main_refine_options {
public:
    main_refine_options(int argc, char** argv) {
        ...
        while(options_left) {
            int option_index, c;
            c = getopt_long(argc, argv, SHORT_OPTIONS, main_refine_long_options, &option_index);
            if(c == -1) {
                options_left = false;
            }
            else {
    ...
    string usage() {
        return string("Sample usage: refine -i A.pdb -i B.pdb -i C.pdb ") +
        "--calpha main_pdb --labels A,B,C \n"
        "\t\t --transforms-file transform-file --transforms-num 100 --output-prefix prefix\n\n"
        "Parameters:\n\t" +
    ...
    vector<pair<string,string> > get_neighbors() {
        ...
        for (size_t i=0; i<neighbors.size(); i++) {
            pair<string, string> current = neighbors[i];
