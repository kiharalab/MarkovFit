...
bool readmrc(MRC *mrc, char *filename, int mapType){
 FILE *fpin;
 int ibuf,res,mode;
 int mapr,mapc,maps;
 int xdim,ydim,zdim;
 ...
 if(mrc->ncstart!=0||mrc->nrstart!=0||mrc->nsstart!=0){
     ...
     if (mapType==1) {//2 simulated
         mrc->orgxyz[0]=mrc->orgxyz[0];
         mrc->orgxyz[1]=mrc->orgxyz[1];
         mrc->orgxyz[2]=mrc->orgxyz[2];
     }
 }
    printf("#3orgXYZ: %f %f %f\n",mrc->orgxyz[0],mrc->orgxyz[1],mrc->orgxyz[2]);
    printf("#width: %f %f %f\n",mrc->widthx,mrc->widthy,mrc->widthz);
...
//Center
void get_center(MRC *m,MRC *M,int t[3],double cen[3]){
    int i,ind;
    int Natm=0;
    double tmp[3];
    double add[3];
    ...
    if(f[0]>0.5*M->zdim) f[0]=f[0]-M->xdim;
    if(f[1]>0.5*M->zdim) f[1]=f[1]-M->xdim;
    if(f[2]>0.5*M->zdim) f[2]=f[2]-M->xdim;
    
    add[0]=m->orgxyz[0]-f[0]*M->widthx;
    add[1]=m->orgxyz[1]-f[1]*M->widthx;
    add[2]=m->orgxyz[2]-f[2]*M->widthx;
