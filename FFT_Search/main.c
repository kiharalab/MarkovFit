...
int main(int argc, char **argv)
{
 ...
 if(readmrc(&mrc1,cmd.file1,cmd.mapType))
  return(0);
 if(readmrc(&mrc2,cmd.file2,cmd.mapType))
  return(0);
...
//Setup 2^x size 
 ...
 //Vector based
 if(cmd.Mode==1)
  if(SearchMAPfftMT(&mrcN1,&mrcN2,cmd.Amode))
   return(0);

 //Overlap based
 if(cmd.Mode==2||cmd.Mode==3||cmd.Mode==4)
  if(SearchMAPfftMT_OVCC(&mrcN1,&mrcN2,cmd.Mode-2,cmd.Amode))
   return(0);
